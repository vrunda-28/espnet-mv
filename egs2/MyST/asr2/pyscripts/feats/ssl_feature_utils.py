...
from espnet2.asr.frontend.s3prl import S3prlFrontend,S3prlFrontendEnsemble
...
class S3PRLFeatureReaderEnemble(BaseFeatureReader):
    def __init__(
        self,
        fs: Union[int, str] = 16000,
        s3prl_conf: Optional[dict] = None,
        ...
        self.device = "cuda" if use_gpu and torch.cuda.is_available() else "cpu"
        self.model = S3prlFrontendEnsemble(
            device = self.device,
            fs=fs,
            frontend_conf=s3prl_conf,
            download_dir=download_dir,
